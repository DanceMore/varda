...
impl VardaApp {
    ...
    pub fn render_outputs(&mut self) {
        ...
        {
            let mut sub_mix_set = std::collections::HashSet::new();
            for surface in &self.output.surface_manager.surfaces {
                if let OutputSource::Channels(indices) = &surface.source {
                    let mut sorted = indices.clone();
                    sorted.sort();
                    sorted.dedup();
                    sub_mix_set.insert(sorted);
                }
            }
            let sub_mix_sources: Vec<Vec<usize>> = sub_mix_set.into_iter().collect();
            let mut sub_mix_cmds = Vec::new();
            self.mixer
                .prepare_sub_mixes(&sub_mix_sources, context, &mut sub_mix_cmds);
