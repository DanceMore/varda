...
fn checked_slice(data: &[u8], start: usize, len: usize) -> Result<&[u8]> {
    let end = start
        .checked_add(len)
        .context("HAP slice range overflow")?;
    data.get(start..end)
        .context("HAP slice out of bounds")
}
...
fn decode_chunked(data: &[u8], output: &mut Vec<u8>) -> Result<()> {
    ...
    let instr = checked_slice(data, h.header_size, h.data_length)?;
    let fdata_start = h.header_size.checked_add(h.data_length).context("HAP frame data overflow")?;
    let fdata = data.get(fdata_start..).context("HAP frame data slice OOB")?;
    let (mut ct, mut st, mut ot): (&[u8], &[u8], Option<&[u8]>) = (&[], &[], None);
    let mut p = 0;
    while p < instr.len() {
        let s = parse_header(&instr[p..])?;
        let d = checked_slice(instr, p + s.header_size, s.data_length)?;
    ...
    if let Some(o) = ot {
        if o.len() < n * 4 {
            bail!("Chunk offset table too short");
        }
    }
    output.clear();
    let mut ao: usize = 0;
    ...
    for i in 0..n {
        ...
        let chunk = checked_slice(fdata, off, sz)?;
        match ct[i] {
...
pub fn decode_hap_frame(
    ...
    let section_data = checked_slice(packet_data, h.header_size, h.data_length)?;
    ...
    if h.section_type == SECTION_MULTI_IMAGE {
        ...
        while pos < section_data.len() {
            ...
            let sub_data = checked_slice(section_data, pos + sub.header_size, sub.data_length)?;
